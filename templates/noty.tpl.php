...
$noty_level = !empty($this->data['noty']['level']) ? $this->data['noty']['level'] : null;
...
if($theme_use !== 'default') {
    ...
    $themeConfig = Configuration::getConfig($theme_config_file);
...
$this->data['head'] = '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">' . PHP_EOL
    . '<link rel="stylesheet" type="text/css" href="/' . $this->data['baseurlpath'] . 'module.php/attrauthcomanage/resources/css/style.css" />' . PHP_EOL;
...
<p>
    ...
    <div class="noty-description">
        <?php
        foreach($this->data['noty']['description'] as $dict_key => $placeholders) {
            if(!empty($placeholders)) {
                print $this->t('{attrauthcomanage:noty:' . $dict_key . '}', $placeholders);
            } else {
                print $this->t('{attrauthcomanage:noty:' . $dict_key . '}');
            }
        }
        ?>
    </div>
...
<?php if(!empty($this->data['yesTarget']) && $yes_button_show): ?>
    <form style="display: inline; margin: 0px; margin-right: 0.5em; padding: 0px"
        ...
        <button type="submit" name="yes" class="btn btn-primary" id="yesbutton">
            <?= htmlspecialchars($yes_button_label) ?>
        </button>
    </form>
<?php endif; ?>
...
<form style="display: inline;"
    ...
    <button type="submit"
            ...
            name="no"
            id="nobutton">
        <?= htmlspecialchars($this->t('{attrauthcomanage:noty:no}')) ?>
    </button>
</form>
...
<script type="text/javascript"
        src="/<?= $this->data['baseurlpath']; ?>module.php/attrauthcomanage/resources/js/noty/jquery.noty.js"></script>
<script type="text/javascript"
        src="/<?= $this->data['baseurlpath']; ?>module.php/attrauthcomanage/resources/js/noty/layouts/topCenter.js"></script>
<script type="text/javascript"
        src="/<?= $this->data['baseurlpath']; ?>module.php/attrauthcomanage/resources/js/noty/themes/comanage.js"></script>
<script type="application/javascript">
    let button_text = '<?= $this->t('{attrauthcomanage:noty:yes}')?>';
    ...
    function form_ajax(frm_ob) {
        ...
        jqxhr.done((data, textStatus, xhr) => {
            ...
            $('.invite-expire').html('<?= htmlspecialchars($this->t('{attrauthcomanage:noty:invitation_restored}')); ?>');
